...
def hotel_cost(num_nights, hotel_star):
    ...
    if hotel_star == 1:
        ...
        night_cost = 120
    hotel_price = night_cost * num_nights
    return hotel_price
...
'''
def car_rental(rental_days, car_make):
    rental_cost = 0 # rental_cost variable initialised to 0
    if car_make in ("ford", "fo", "for"):
        ...
        rental_cost = 150 * rental_days
    elif car_make in ("ferrari", "fe", "fer"):
...
# User input to choose where they are flying to
while True:
...
while True:
    ...
    # if the user inputs one of the below strings, the choice is validated and breaks out of the loop
    if car_make in ("ford", "fo", "for", "volkswagen", "v", "vol", "jaguar", "j", "jag", "mercedes", "m", "mer",
                    "ferrari", "fe", "fer"):
        break
    # If one of the strings isn't typed by the user, it is invalid and will have to try again until it is a valid choice
    else:
...
# User input to choose how long they'd like the rental car for
while True:
    try:
