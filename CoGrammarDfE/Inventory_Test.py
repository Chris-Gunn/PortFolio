...
def read_data():
    ...
    with open('inventory.txt', 'r') as file:
        ...
        # Removes all comma's from file and stores in data
        for lines in file:
            data = lines.strip().split(',')

            # Top line is column names, 'Country' is the first column name.
            # Removes all the column names, so just data
            if "Country" not in data:
                # Adds data to shoe_data and each part of the line relates to each part of the shoe class
                # data[0] = country, data[1] = code, data[2] = prod, data[3] = cost, data[4]
